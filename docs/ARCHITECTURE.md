...
- **Open team sheets — expected YES:** the Reg M-B ruleset includes "Open Team Sheets", so `|showteam|` should appear in official-ladder replays. Confirm against a real replay fixture in build step 3 — TODO(confirm).
...
- `search.json` combined `user+format` filtering in one query? — TODO (test in build step 2).
