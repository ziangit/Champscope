...
- **Team-source ingest:** `/api/ingest/run` (same bearer + chunked-cursor design), driven daily (06:45 UTC) by GH Actions `team-ingest` (needs variable `INGEST_URL`); sources run as separate passes (vgcpastes, pokedata, metavgc), 20 h cooldown each. The workflow loops until `cooldown` — `finished` only ends one source. Local CLI: `npx tsx scripts/dev-ingest.ts`. Idempotent per source key; a re-run mostly no-ops.
